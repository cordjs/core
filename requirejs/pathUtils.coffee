  ...
  class PathUtils
    ...
    parsePathRaw: (path) ->
      ...
      if nameParts.length > 2
        throw new Error("Not more than one @ is allowed in the widget name specification: #{path}!")
      if nameParts.length == 2
      ...
      if path.indexOf('/') == -1
        ...
        nameParts = path.split '//'
        if nameParts.length > 2
          throw new Error("Not more than one // is allowed in widget name specification: #{ path }!")
        if nameParts.length == 2
          ns = nameParts[0]
