define [
  ...
  class ServerSideRouter extends Router
    ...
    _prepareConfigForRequest: (request) ->
      ...
      serverProto = Utils.substituteTemplate(serverProto, context.templates)
      backendProto  = Utils.substituteTemplate(backendProto, context.templates)
      ...
      if global.appConfig.browser.xdr
        xdr = Utils.substituteTemplate(global.appConfig.browser.xdr, context.templates)
      else
        xdr = serverProto + '://' + serverHost + (if serverPort then ':' + serverPort else '') + '/XDR/'

      if global.appConfig.node.backend.host
        backend = Utils.substituteTemplate(global.appConfig.node.backend.host, context.templates)
      else
